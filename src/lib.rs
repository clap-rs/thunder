...
pub fn thunderclap(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let mut accessors : Vec<ImplItem> = vec![];
    ...
    global_args.iter().for_each(|(name, typed, about)| {
        ...
        accessors.push(syn::parse(quote! {
            #[allow(unused)]
    ...
    let mut impl_items = i.items.clone();  // Add to existing impl items rather than create additional impl block.

    let start_method : ImplItem = syn::parse(quote!(
            /// Starts the CLI parsing and calls whichever function handles the input surprises
            fn start() {
    ...
    impl_items.push(start_method);
    impl_items.extend(accessors);

    let original_impl_extended = ItemImpl{items:impl_items, .. i};

    let tokens = quote! {
        #original_impl_extended
